...
* KD240 power supply and adapter (included with the KD240 Drives Starter Kit)
  * 12V AC adapter for the KD240 Starter Kit and 24V AC adapter for the Motor
      Accessory Kit
...
* Load the firmware:
  ...
  * Show the list and status of available application firmware.
    ...
    captured under the listapps function and to have dfx-mgrd rescan and
  ...
  * Load the desired application firmware.
    ...
    When there is already another accelerator/firmware loaded, unload it
...
motor control knowledge and experience. Incorrect configurations of values of Vd and Vq
...
* The Sample Size text box is used to indicate how many samples are collected and
  ...
  there might be a small delay before a dashboard command takes effect.
* The Refresh Interval text box is used to indicate how often the dashboard
  plots will refresh. A minimum refresh interval will be enforced
...
* The Open Loop - Vd text box is used to set the  direct voltage (Vd).
  The valid range for Vd is -24 to 24 volts.
...
* The Electrical Data plot shows the currents and voltages for Phase A, B, and
  ...
  and voltage line can be toggled by clicking the legend labels. The current
...
To run the application in Speed mode, select **Speed** from the Mode drop-down, and
use the Speed Setpoint text box to enter a speed setpoint. The following image
...
To run the application in Torque mode, select **Torque** from the Mode drop-down and
use the Torque Setpoint text box to enter a torque setpoint. The following image
...
drop-down, and use the Vd/Vq text boxes to set Vd/Vq. The following image shows the
...
The following images show what the dashboard looks like when a larger load is
